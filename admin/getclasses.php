...
if($material_id != ""){
  ...
  $sql = "SELECT * FROM classes WHERE school_id='$school_id'";
    ...
    ?>
    <option value="<?php echo "$row[id]"; ?>" data-value="<?php echo "$rowt[name] "; ?><?php if("$row[div_id]" != ""){
        $sqls = "SELECT * FROM divs WHERE id='$row[div_id]'";
        $results = $conn->query($sqls);
        if ($results->num_rows > 0) {
          while($rows = $results->fetch_assoc()) {
           echo "$rows[name] ";}} $conn->close(); } ?><?php echo "$row[name]"; ?>"><?php echo "$rowt[name] "; ?><?php if("$row[div_id]" != ""){
        include('../db.php');
        $sqls = "SELECT * FROM divs WHERE id='$row[div_id]'";
        $results = $conn->query($sqls);
        if ($results->num_rows > 0) {
