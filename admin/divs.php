                ...
                <div class="mt-4 mb-4 p-3 bg-white border shadow-sm lh-sm">
                    ...
                    <div class="product-list">
                        ...
                        <div class="row border-bottom mb-4">
                            <div class="col-sm-8 pt-2"><h6 class="mb-4 bc-header"><i class="fas fa-network-wired"></i> <?php echo $divs; ?></h6></div>
                            <div class="col-sm-4 text-right pb-3">
                                <button class="btn btn-outline-theme shadow" data-toggle="modal" data-target="#addDiv" onclick="getyears('adschool', 'adyear')">
                                    <i class="fas fa-plus"></i> <?php echo $adddiv; ?>
                                </button>
                            </div>
                        </div>
...
<div class="col-sm-6">
<select class="form-control" id="syear_id">
...
<?php } ?>
                        ...
                        <table class="table table-bordered table-striped mt-0" width="100%" id="divs">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th><?php echo $name; ?></th>
...
$sql = "SELECT * FROM divs WHERE school_id='$_COOKIE[school_id]'";
}
$i = 1;
$result = $conn->query($sql);
...
<td class="align-middle text-center">
    ...
    <button class="btn btn-danger" onclick="deldiv(<?php echo "$row[id]"; ?>)"><i class="fas fa-trash"></i></button>
</td>
</tr>
<?php
  }}
...
?>
                    ...
                    <!--Div Add Modal-->
                    <div class="modal fade" id="addDiv" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle"><?php echo $adddiv; ?></h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <input id="adid" hidden>
                                    ...
                                    <select class="form-control" id="adschool" onchange="getyears('adschool', 'adyear')">
                                        <option value=""><?php echo $school; ?></option>
<?php
include('../db.php');
$sql = "SELECT * FROM schools";
...
?>
                                    ...
                                    <select class="form-control" id="adyear">
                                        <option value=""><?php echo $year; ?></option>
                                    </select>
                                </div>
                                <div class="modal-footer">
                                <button class="btn btn-outline-success" id="adddivbtn" onclick="adddiv()"><i class="fas fa-save"></i> <?php echo $save; ?></button>
...
var school_id = document.getElementById('adschool').value;
var year_id = document.getElementById('adyear').value;
...
if(name == '' || school_id == '' || year_id == '') {
    ...
    $.ajax({
        ...
        cache: false,
        beforeSend: function(){
            document.getElementById('adddivbtn').disabled = true;
            document.getElementById('adddivbtn').innerHTML = "<i class='fa fa-spinner fa-spin'></i>";
        },
        success: function(dataResult){
...
function showmore(id) {
  $.ajax({
        ...
        success: function(dataResult){
            var dataResult = JSON.parse(dataResult);
                $('#addDiv').modal('show');
                $('#adid').val(dataResult.id);
                $('#adname').val(dataResult.name);
                $('#adschool').val(dataResult.school_id);
                getyears('adschool', 'adyear');
                setTimeout(function(){$('#adyear').val(dataResult.year_id);}, 500);
        }
  });
}
function deldiv(id) {
