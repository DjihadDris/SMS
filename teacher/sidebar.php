...
<i class="hand point right outline icon circular small"></i> <?php include('../db.php'); $school_id=$_COOKIE['school_id']; $sql = "SELECT * FROM schools WHERE id='$school_id'"; $result = $conn->query($sql); if ($result->num_rows > 0) {while($row = $result->fetch_assoc()) {echo "$row[name]";}} else {echo "No school found..";} $conn->close(); ?>
  ...
  <a class="item" onclick="openmsgslist()">
    <i class="icon mail large"></i> <?php echo $messagetitle; ?><?php include('../db.php'); $query = "SELECT * FROM messages WHERE to_user_id='$_COOKIE[id]' AND to_type='teachers' AND vu=''"; $result = mysqli_query($conn, $query); $rowCount = mysqli_num_rows($result); if($rowCount > 0){echo "<span class='ui red label'>".$rowCount."</span>";} $conn->close(); ?>
  </a>
...
<div class="ui vertical blue inverted sidebar labeled icon menu" style="background-color: <?php echo $bgcolor; ?>; border: <?php echo $bordersize; ?> <?php echo $bordercolor; ?> <?php echo $borderweight; ?>;">
  ...
  <a class="item" href="details">
    ...
    <a class="item" onclick="showchatclasses()">
...
function showchatclasses(){
  $.ajax({
        url: "getdiscussionclasses.php",
        type: "POST",
        cache: false,
...
;
  ...
  function checkchat(){
    var chatclass_id = document.getElementById('chatclass_id').value;
    $.ajax({
        url: "checkdiscussion.php",
        type: "POST",
        data: {
          class_id: chatclass_id
        },
        cache: false,
        success: function(dataResult){
  eval(dataResult);
