...
<input type="text" placeholder="<?php echo $material; ?>" required="" value="<?php include('../db.php'); $material_id=$_COOKIE['material_id']; $sqls = "SELECT * FROM materials WHERE id='$material_id'"; $results = $conn->query($sqls); if ($results->num_rows > 0) {while($rows = $results->fetch_assoc()) {echo "$rows[name]";}}else{echo "No material found..";} $conn->close(); ?>" readonly="">
<input type="hidden" name="material" value="<?php echo "$row[material_id]" ?>">      
</div>
